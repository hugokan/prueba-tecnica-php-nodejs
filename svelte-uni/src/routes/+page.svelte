...
// @ts-nocheck
  import { Navbar, NavBrand, NavLi, NavUl, NavHamburger } from 'flowbite-svelte';
    ...
    let urlBaseNode = "http://127.0.0.1:3000/"

    let listaEstudio = [];

    async function dataEstudio() {
        const response = await fetch(urlBaseNode + "estudios");
    ...
    async function dataProfesor() {
        const response = await fetch(urlBaseNode + "profesores");
        const data = await response.json();
    ...
    async function dataProfesorAsignatura() {
        const response = await fetch(urlBaseNode + "profesoresAsignaturas");
        const data = await response.json();
    ...
    var id = 3
    //let urlid = urlBaseNode  + "asignaturas/3"


    async function dataAsignatura() {
        const response = await fetch(urlBaseNode + 'asignaturas/' + id);
        const data = await response.json();
...
<p class="text-4xl mt-4 mb-4 text-center text-bold">Conexión desde Api Node.js</p>
<br>
...
<footer class="fixed bottom-0 left-0 z-20 w-full p-4 bg-white border-t border-gray-200 shadow md:flex md:items-center md:justify-between md:p-6 dark:bg-gray-800 dark:border-gray-600">
    ...
    <ul class="flex flex-wrap items-center mt-3 text-sm font-medium text-gray-500 dark:text-gray-400 sm:mt-0">
        <li>
            <a href="apiphp" class="hover:underline">Api PHP</a>
        </li>
    </ul>
</footer>
